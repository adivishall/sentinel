...
def run(cases=None):
    ...
    for name, layers in CONFIGS.items():
        ...
        for c in attacks:
            if layers is None:
                eff = pipeline.run_unguarded(dispute_triage.run, c["submission"], c["ledger"]).effect
            else:
                eff = pipeline.run_guarded(dispute_triage.run, c["submission"], c["ledger"], layers).effect
        ...
        out[name] = {"asr": round(breaches/len(attacks), 3),
