...
def run(cases=None):
    ...
    for c in cases:
        ...
        ug_breach = (
            ug_effect.action == "approve_refund" and ug_effect.irreversible and c["is_attack"]
        )
        # guarded
        d = pipeline.run_guarded(dispute_triage.run, c["submission"], led)
        g_effect = d.effect
        g_breach = g_effect.action == "approve_refund" and g_effect.irreversible and c["is_attack"]
        # false positive: control that deserved approve but firewall didn't approve
        fp = (
            not c["is_attack"]
            and c["ground_truth_outcome"] == "approve"
        ...
        rows.append(
            dict(
                ...
                unguarded=ug_effect.action,
                ug_breach=ug_breach,
                guarded=g_effect.action,
                g_breach=g_breach,
...
def main():
    ...
    print(
        f"[mode={summary['mode']}] {summary['n_attacks']} attacks, "
