...
SIGNALS = [
    (
        ...
        0.9,
        re.compile(
            r"(ignore|disregard|override|forget).{0,40}"
    ...
    (
        ...
        0.8,
        re.compile(
            r"(issue|process|approve|grant|authorli?ze|release|refund)"
            r".{0,30}(the )?(full |immediate |complete )?(refund|amount|₹|rs)",
            re.I | re.S,
        ),
    ),
    (
        ...
        re.compile(
            ...
            r"|priority[- ]?override|internal use only",
            re.I | re.S,
        ),
    ),
    (
        ...
        0.7,
        re.compile(
            r"per (network |mastercard )?rule \d|regulation \w+ (requires|mandates)"
            r"|section \d+\.\d+.{0,20}(requires|mandates|refund)",
            re.I | re.S,
        ),
    ),
    (
        ...
        0.6,
        re.compile(
            r"you are now|from now on you|pretend (you|to)|new persona|developer mode", re.I
        ),
    ),
    (
        "hidden_channel",
        0.65,
        re.compile(
            r"</?(system|assistant|instruction|admin)>|\[/?(inst|system)\]" r"|```system|<\|.*?\|>",
            re.I,
        ),
    ),
]
...
def score(untrusted_text: str) -> tuple[float, list[dict]]:
    ...
    for name, weight, rx in SIGNALS:
        ...
        if m:
            hits.append({"signal": name, "weight": weight, "span": m.group(0)[:120].strip()})
